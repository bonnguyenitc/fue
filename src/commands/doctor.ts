 ...
 * to run fue properly. This needs some TLC, as it is mostly designed
 * for the old fue CLI and Bowser. fue v4 ("flame") is a combination of the two.
 */
...
module.exports = {
  ...
  run: async function (toolbox: GluegunToolbox) {
    ...
    // -=-=-=- fue -=-=-=-
    const ignitePath = which("fue")
    const igniteSrcPath = `${meta.src}`
    const igniteVersion = meta.version()

    info("")
    info(colors.cyan("fue"))
    const igniteTable = []
    igniteTable.push([column1("fue-cli"), column2(igniteVersion), column3(ignitePath)])
    igniteTable.push([
      column1("fue src"),
